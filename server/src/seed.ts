...
async function main() {
  ...
  const dossiers = await Promise.all(
    dossierData.map(d => prisma.dossier.create({ data: d }))
  );
  ...
  for (let i = 0; i < 40; i++) {
    ...
    await prisma.timeEntry.create({
      data: {
        datum: daysAgo(Math.floor(Math.random() * 30)),
  ...
  const invoice = await prisma.invoice.create({
    data: {
      nummer: 'RE-2025-01-0042',
      status: 'VERSENDET',
      faelligkeitsdatum: daysAhead(30),
      betrag: 4500,
      mwst: 7.7,
  ...
  await prisma.invoice.create({
    data: {
      nummer: 'RE-2025-01-0043',
      status: 'BEZAHLT',
      ausstellungsdatum: daysAgo(45),
      faelligkeitsdatum: daysAgo(15),
      betrag: 2100,
      mwst: 7.7,
      dossierId: dossiers[8].id,
  ...
  await prisma.invoice.create({
    data: {
      nummer: 'RE-2025-01-0044',
      status: 'UEBERFAELLIG',
      ausstellungsdatum: daysAgo(60),
      faelligkeitsdatum: daysAgo(30),
      betrag: 3750,
      mwst: 7.7,
      dossierId: dossiers[7].id,
  ...
  const packages = await Promise.all([
    prisma.legalPackage.create({
      data: {
        ...
        beschreibung: 'Vollständiges Paket für familienrechtliche Mandate inkl. Scheidung, Unterhalt, Kindesrecht',
        aktiv: true,
        templates: {
          create: [
            { name: 'Scheidungsklage', inhalt: 'An das Bezirksgericht...\n\nKlageschrift betreffend Scheidung\n\nDie Parteien:\n- Kläger/in: {{klaeger}}\n- Beklagte/r: {{beklagter}}\n\nSachverhalt:\n{{sachverhalt}}\n\nRechtliche Grundlage: Art. 114 ff. ZGB\n\nBegehren:\n1. Die Ehe der Parteien sei zu scheiden.\n2. ...', typ: 'Schriftsatz' },
            { name: 'Unterhaltsberechnung', inhalt: 'Unterhaltsberechnung gemäss Zürcher Tabellen\n\nBarunterhalt Kinder:\n{{berechnung}}\n\nNettoeinkommen Pflichtige/r: CHF {{einkommen}}\nZumutbares Einkommen: CHF {{zumutbar}}\n\nGrundlage: BGE 147 III 265', typ: 'Checkliste' },
            { name: 'Eheschutzgesuch', inhalt: 'EHESCHUTZGESUCH\n\nAn das Gericht:\n...\n\nBegehren:\n1. Es sei dem Gesuchsteller/der Gesuchstellerin zu gestatten, den gemeinsamen Haushalt aufzuheben.\n2. Die Kinder {{kinder}} seien unter die Obhut des Gesuchstellers/der Gesuchstellerin zu stellen.', typ: 'Schriftsatz' },
          ],
        },
      },
    }),
    prisma.legalPackage.create({
      data: {
        ...
        beschreibung: 'Erbrecht-Paket: Testamente, Erbverträge, Nachlassregelung',
        aktiv: true,
        templates: {
          create: [
            { name: 'Testament', inhalt: 'LETZTER WILLE\n\nIch, {{name}}, geboren am {{geburtsdatum}}, in {{ort}},\nbestimme hiermit meinen letzten Willen:\n\n1. Mein gesamtes Vermögen vermache ich...\n\nDatum: {{datum}}\nUnterschrift: _______________', typ: 'Vorlage' },
            { name: 'Pflichtteilsklage', inhalt: 'KLAGE\n\nAn das Bezirksgericht:\n\nBetreff: Herabsetzungsklage (Art. 522 ZGB)\n\nKläger/in: {{klaeger}}\nBeklagte/r: {{beklagter}}\n\nDer Erblasser {{erblasser}} ist am {{todesdatum}} verstorben.\n\nDie Klägerin/Der Kläger ist pflichtteilsberechtigt.', typ: 'Schriftsatz' },
          ],
        },
      },
    }),
    prisma.legalPackage.create({
      data: {
        ...
        beschreibung: 'Arbeitsrecht: Kündigungen, Lohnklagen, Diskriminierung',
        aktiv: true,
        templates: {
          create: [
            { name: 'Kündigungsschutzklage', inhalt: 'KLAGE\n\nAn das Arbeitsgericht:\n\nKläger/in: {{arbeitnehmer}}\nBeklagte/r: {{arbeitgeber}}\n\nKlagegegenstand: Ungerechtfertigte Kündigung\n\nGrundlage: Art. 336 ff. OR\n\nBegehren:\n1. Die Kündigung vom {{datum}} sei als missbräuchlich zu erklären.\n2. Die Beklagte sei zu verpflichten, dem Kläger eine Entschädigung von CHF {{betrag}} zu bezahlen.', typ: 'Schriftsatz' },
            { name: 'Arbeitszeugnis Kontrolle', inhalt: 'CHECKLISTE ARBEITSZEUGNIS\n\n□ Vollständigkeit der Personalien\n□ Dauer des Arbeitsverhältnisses\n□ Beschreibung der Tätigkeit\n□ Leistungsbeurteilung\n□ Verhaltensbeurteilung\n□ Schlussformel (wohlwollend)\n□ Datum und Unterschrift', typ: 'Checkliste' },
          ],
        },
      },
    }),
    prisma.legalPackage.create({
      data: {
        ...
        beschreibung: 'Mietrecht: Kündigungen, Mietzinsreduktion, Mängel',
        aktiv: false,
        templates: {
          create: [
            { name: 'Mietzinsreduktionsbegehren', inhalt: 'MIETZINSREDUKTIONSBEGEHREN\n\nAn: {{vermieter}}\n\nBetreff: Mietvertrag {{adresse}}\n\nSehr geehrte Damen und Herren,\n\ngestützt auf Art. 259a OR verlange ich eine Mietzinsreduktion von {{prozent}}% aufgrund folgender Mängel:\n\n{{maengel}}\n\nIch bitte Sie um schriftliche Stellungnahme bis {{frist}}.', typ: 'Schriftsatz' },
          ],
        },
      },
    }),
    prisma.legalPackage.create({
      data: {
        ...
        beschreibung: 'Vertragsrecht: Kauf, Werkvertrag, Auftrag, AGB',
        aktiv: false,
        templates: {
          create: [
            { name: 'Kaufvertrag', inhalt: 'KAUFVERTRAG\n\nZwischen:\n- Verkäufer: {{verkaeufer}}\n- Käufer: {{kaeufer}}\n\nGegenstand: {{gegenstand}}\nKaufpreis: CHF {{preis}}\n\nZahlungsbedingungen: {{zahlungsbedingungen}}\n\nGewährleistung: Gemäss Art. 197 ff. OR', typ: 'Vorlage' },
          ],
        },
      },
    }),
